 ...
 * Clean flame — no glow circle behind it. Just the flame itself with a
 * soft drop-shadow and three animated layers that flicker at different
 * speeds so the fire looks alive and natural.
 */
...
/* How long (ms) each spotlight image stays on screen before sliding to
 * the next image (and, eventually, to the next tile). */
...
/* Images render with object-fit: cover and center alignment, so photos
   appear upright, uncropped, and never stretched. */
const PhotoSlider = ({
  images = [],
  alt = '',
  className = '',
...
/* ---------- Spotlight tile ----------
 * Taller than before (4 / 5.6 aspect ratio) and auto-plays its own images
 * every SPOTLIGHT_IMAGE_MS milliseconds. When a tile finishes its last
 * image, the parent advances the scroll to the next tile.
 */
const SpotlightTile = ({
  item,
  onOpen,
  ...
  // Notify the parent when this tile has cycled through every image.
  useEffect(() => {
    if (total <= 1) return;
    const t = setTimeout(() => {
      setIdx((prev) => {
...
const Landing = () => {
   ...
   * Each tile tells us when it has finished cycling through its own
   * images; when that happens, we smoothly scroll the strip to the
   * next tile. When the last tile finishes, we scroll back to the first.
   */
  const scrollSpotlightTo = useCallback((index) => {
    ...
    container.scrollTo({
      left: tile.offsetLeft - 20, // match the strip's left padding
      behavior: 'smooth',
    });
  }, []);
  ...
  useEffect(() => {
    ...
    // Fallback ticker so even single-image tiles get their turn.
    // It advances one tile every SPOTLIGHT_IMAGE_MS * (avg images) — but
    // since we can't know that, we just use a slightly longer interval
    // and rely on the tile-level callbacks to trigger the fast path when
    // a tile has multiple images.
  ...
  return (
    <div className="app">
          ...
          {freshListings.length > 0 && (
            <section className="section">
              <header className="section-head">
                ...
                {/* Right-hand fire removed — only the left flame remains */}
              </header>
              <div className="grid">
      ...
      <style jsx>{`
        ...
        /* ---------- Live burning fire (cleaner, no glow circle) ---------- */
        .burning-fire {
          position: relative;
          display: inline-flex;
          align-items: center;
